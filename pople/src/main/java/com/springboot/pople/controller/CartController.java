...
import com.springboot.pople.dto.item.CartOrderDTO;
...
//        }
   ...
   /* @PostMapping(value = "")
    public @ResponseBody ResponseEntity orderCartItem(@RequestBody CartOrderDTO cartOrderDTO,Principal principal){
        // 클라이언트로 부터 넘겨받은 장바구니 상품목록
        log.info("장바구니 주문 상품 : "+cartOrderDTO.toString());

    List<CartOrderDTO> cartOrderDTOList = cartOrderDTO.getCartOrderDTOList();

    if(cartOrderDTOList == null || cartOrderDTOList.size() == 0){
        log.info("장바구니 주문상품에서 체크항목이 없을시");

        return  new ResponseEntity<String>("주문할 상품을 선택해주세요",HttpStatus.FORBIDDEN);
    }

        for(CartOrderDTO cartOrderItem: cartOrderDTOList){
            // 로그인한 회원의 장바구에 있는 주문 상품 주문자 인지 확인
